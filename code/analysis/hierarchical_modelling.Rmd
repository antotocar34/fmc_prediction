...
get_formula <- function(df, shared_fx, exclude, select=c(),  type){
  xs = paste(names(df)[-which(names(df) %in% exclude)], collapse = " + ")
  ...
  if (type=="fixest_RE"){
    ...
    if (is_empty(shared_fx)==TRUE){
      formula <- as.formula(paste(target, "1 | Fuel[", select, "]", collapse = " "))
    }
    else{
      formula <- as.formula(paste(target, shared, "| Fuel[", select, "]", collapse = " "))
    }
  }
    ...
    if ("all" %in% select){
      formula <- as.formula(paste(target, paste(c(shared, xs), collapse= "+"), "+ (1 +", paste(xs, collapse="+"), "| Fuel)", collapse = " "))
    }
    else{
...
```{r}
setwd(PATH)
data = read.csv("data/processed_data/socc_synoptic_complete.csv")
...
select = c("precip_MEAN15",  "solar_rad_max_MEAN7", "DC") #For LMER Random effects, only these features will be used. If "all" is passed, then all available features will be used. 
...
```
```{r}
FE_formula <- get_formula(train, exclude=exclude_from_FE, shared_fx=shared_fx, type="fixest_FE")
RE_formula <- get_formula(train, exclude=exclude_from_FE, shared_fx=shared_fx, select=select, type="fixest_RE")
bayes_formula <- get_formula(train, exclude=exclude_from_FE, shared_fx=shared_fx, type="bayes")
lmerFE_formula <- get_formula(train, exclude=exclude_from_FE, shared_fx=shared_fx, type="lmer_FE")
lmerRE_formula <- get_formula(train, exclude=exclude_from_FE, shared_fx=shared_fx, select=select, type="lmer_RE")
```
```{r message=FALSE}
